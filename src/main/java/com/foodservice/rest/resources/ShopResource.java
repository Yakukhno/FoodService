...
public class ShopResource {
    ...
    public Response create(Shop shop, @QueryParam("shopAdminUserID") int shopAdminUserID) {
        System.out.println(shop);
        try {
            Shop shop1 = shopService.create(shop, shopAdminUserID);
    ...
    @GET
    @Path("/byShopAdminID/{shopAdminUserID}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getByManagerID(@PathParam("shopAdminUserID") Integer shopAdminUserID) {
        List<Shop> result = shopService.getByShopAdminID(shopAdminUserID);
        if (result.size() != 0) {
